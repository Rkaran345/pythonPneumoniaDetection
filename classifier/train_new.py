...
class Train:
    def __init__(self, force_download=False):
        ...
        self.epochs = 10  # Increased number of epochs
    ...
    def train_model(self):
        self.load_data()
        steps_per_epoch = max(1, self.train.samples // self.train.batch_size)
        validation_steps = max(1, self.val.samples // self.val.batch_size)
        self.model = self.define_model()
        
        # Implementing early stopping
        early_stopping = EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)  # Fixed import
